...
const MOODS=[
  ...
  {val:4,emoji:"🙂",label:"不错",color:"#2563eb"},
  {val:3,emoji:"😐",label:"一般",color:"#d97706"},
  {val:2,emoji:"😔",label:"低落",color:"#ea580c"},
  {val:1,emoji:"😢",label:"很差",color:"#dc2626"},
];
...
function Toast({msg,onClose}){
  ...
  return <div style={{position:"fixed",top:16,right:16,zIndex:9999,background:T.text,color:"#fff",borderRadius:T.radius,padding:"12px 20px",boxShadow:T.shadowMd,display:"flex",alignItems:"center",gap:10,fontSize:13,maxWidth:320,animation:"slideIn .3s ease-out"}}>
...
function NewTagModal({onClose,onCreate}){
  ...
  return <Modal onClose={onClose}><div style={{background:T.card,borderRadius:16,padding:28,width:"min(380px,calc(100vw - 32px))",border:`1px solid ${T.cardBorder}`,boxShadow:T.shadowMd,maxHeight:"82vh",overflowY:"auto"}}>
    ...
    <div style={{display:"flex",gap:10}}>
      <button onClick={onClose} style={{flex:1,background:T.accentSoft,border:`1px solid ${T.divider}`,borderRadius:T.radiusSm,padding:"10px 0",color:T.textSec,cursor:"pointer",fontSize:13,fontWeight:500}}>取消</button>
...
function EditTodoModal({todo,tags,onSave,onClose}){
  const[t,sT]=useState(todo.text);const[st,sST]=useState(todo.tags||[]);const[r,sR]=useState(todo.reminder||"");
  const tog=id=>sST(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]);
  return <Modal onClose={onClose}><div style={{background:T.card,borderRadius:16,padding:28,width:"min(380px,calc(100vw - 32px))",border:`1px solid ${T.cardBorder}`,boxShadow:T.shadowMd}}>
    ...
    <input value={t} onChange={e=>sT(e.target.value)} placeholder="待办内容" style={{width:"100%",background:T.accentSoft,border:`1px solid ${T.divider}`,borderRadius:T.radiusSm,padding:"10px 14px",color:T.text,fontSize:14,outline:"none",marginBottom:14,boxSizing:"border-box"}}/>
    ...
    <div style={{display:"flex",gap:10}}>
      ...
      <button onClick={()=>{onSave({...todo,text:t.trim()||todo.text,tags:st,reminder:r||null});onClose()}} style={{flex:1,background:T.text,border:"none",borderRadius:T.radiusSm,padding:"10px 0",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600}}>保存</button>
    </div>
  </div></Modal>;
}
...
function MiniBar({vals,colors,labels,h=70}){
  ...
  return <div style={{display:"flex",alignItems:"flex-end",gap:6,height:h}}>
    {vals.map((v,i)=><div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flex:1}}>
      ...
      <div style={{width:"100%",maxWidth:28,height:Math.max((v/mx)*h*.65,2),background:colors?.[i]||T.text,borderRadius:4,transition:"height .3s"}}/>
...
function SummaryView({sm,tags,label}){
  if(!sm)return<div style={{textAlign:"center",padding:50,color:T.textTer}}><div style={{fontSize:48,marginBottom:10}}>📭</div><p>{label}暂无数据</p></div>;
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {sm.topTags.length>0&&<SC title="高频标签"><div style={{display:"flex",flexDirection:"column",gap:8}}>
      {sm.topTags.map(([tid,cnt],i)=>{const tg=tags.find(t=>t.id===tid);const mx=sm.topTags[0][1];return tg?<div key={tid} style={{display:"flex",alignItems:"center",gap:8}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      {sm.bestDay&&<SC title="🌟 最佳一天"><div style={{fontSize:13,fontWeight:600,color:"#059669"}}>{sm.bestDay.date}</div><div style={{fontSize:11,color:T.textSec,marginTop:3}}>{sm.bestDay.score}/10 {MOODS.find(m=>m.val===sm.bestDay.mood)?.emoji}</div>{sm.bestDay.diary&&<div style={{fontSize:11,color:T.textTer,marginTop:3,lineHeight:1.5}}>{sm.bestDay.diary.slice(0,50)}...</div>}</SC>}
      {sm.worstDay&&<SC title="💪 需加油"><div style={{fontSize:13,fontWeight:600,color:"#d97706"}}>{sm.worstDay.date}</div><div style={{fontSize:11,color:T.textSec,marginTop:3}}>{sm.worstDay.score}/10 {MOODS.find(m=>m.val===sm.worstDay.mood)?.emoji}</div>{sm.worstDay.diary&&<div style={{fontSize:11,color:T.textTer,marginTop:3,lineHeight:1.5}}>{sm.worstDay.diary.slice(0,50)}...</div>}</SC>}
    </div>
  </div>;
}

function EntryCard({dateKey,e,hl,tags:tgs}){
  ...
  return <div style={{background:T.card,borderRadius:T.radius,padding:18,border:`1px solid ${T.cardBorder}`,boxShadow:T.shadow,borderLeft:hl?`3px solid ${hl}`:"none"}}>
    ...
    {e.diary&&<p style={{margin:"4px 0 10px",fontSize:13,lineHeight:1.7,color:T.textSec}}>{e.diary}</p>}
    {e.todos?.length>0&&<div style={{display:"flex",flexDirection:"column",gap:3,marginBottom:8}}>
      {e.todos.map((t,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:t.done?T.textTer:T.text}}>
        ...
        {t.reminder&&<span style={{fontSize:10,color:"#2563eb",fontWeight:500}}>⏰{t.reminder}</span>}
      </div>)}
    </div>}
...
export default function LifeLogApp({
  data, tags, updateEntry, addTodo, toggleTodo, removeTodo, saveTodo, addTag, onSignOut, userEmail
}){
  const[yr,sYr]=useState(2026);
  const[mo,sMo]=useState(2);
  const[sel,sSel]=useState(null);
  const[view,sView]=useState("calendar");
  const[fTag,sFTag]=useState(null);
  const[fMood,sFMood]=useState(null);
  const[fScore,sFScore]=useState(null);
  const[eDiary,sEDiary]=useState(false);
  const[nTodo,sNTodo]=useState("");
  const[showNT,sSNT]=useState(false);
  const[eTIdx,sETIdx]=useState(null);
  const[toasts,sToasts]=useState([]);
  const[sMonth,sSMonth]=useState("2026-02");
  const[sYear,sSYear]=useState("2026");
  const fired=useRef(new Set());

  const now=new Date();
  useEffect(()=>{sYr(now.getFullYear());sMo(now.getMonth()+1);sSMonth(`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`);sSYear(String(now.getFullYear()))},[]);
  ...
  const key=sel;const entry=key?data[key]:null;
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{const n=new Date();const nk=dkFn(n.getFullYear(),n.getMonth()+1,n.getDate());
      ...
      data[nk]?.todos?.forEach((t,i)=>{if(t.reminder&&t.reminder===hm&&!t.done){const uid=`${nk}-${i}-${t.reminder}`;
        if(!fired.current.has(uid)){fired.current.add(uid);sToasts(p=>[...p,{id:Date.now(),msg:`⏰ ${t.text}`}])}}})},10000);
  ...
  const handleAddTodo=()=>{if(!nTodo.trim()||!sel)return;addTodo(sel,nTodo.trim());sNTodo("")};
  const handleToggle=i=>toggleTodo(sel,i);
  const handleRemove=i=>removeTodo(sel,i);
  const handleSaveTodo=(i,u)=>saveTodo(sel,i,u);

  const getDots=dk2=>{const e=data[dk2];if(!e?.todos)return[];const s=new Set();e.todos.forEach(t=>t.tags?.forEach(tid=>s.add(tid)));return[...s].slice(0,4).map(tid=>tags.find(t=>t.id===tid)?.color||"#999")};

  const navItems=[{id:"calendar",l:"📅 日历"},{id:"tags",l:"🏷️ 标签"},{id:"mood",l:"😊 心情"},{id:"score",l:"⭐ 评分"},{id:"monthly",l:"📊 月报"},{id:"yearly",l:"📈 年报"}];

  const Pill=({active,color,children,onClick})=><button onClick={onClick} style={{background:active?(color?`${color}10`:T.text):T.accentSoft,color:active?(color||"#fff"):T.textSec,border:active&&color?`1.5px solid ${color}40`:`1px solid ${T.divider}`,borderRadius:20,padding:"6px 16px",cursor:"pointer",fontSize:12,fontWeight:active?600:400,transition:"all .2s"}}>{children}</button>;

  const Nav=()=><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
    <div style={{display:"flex",alignItems:"center",gap:12}}>
      <h1 onClick={()=>sView("calendar")} style={{margin:0,fontSize:22,fontWeight:700,color:T.text,cursor:"pointer",letterSpacing:"-0.02em"}}>LifeLog</h1>
    ...
    <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
      {navItems.map(v=><button key={v.id} onClick={()=>{sView(v.id);if(v.id==="tags"&&!fTag)sFTag(tags[0]?.id);if(v.id==="mood"&&!fMood)sFMood(5);if(v.id==="score"&&!fScore)sFScore([8,10])}}
        style={{background:view===v.id?T.text:T.accentSoft,color:view===v.id?"#fff":T.textSec,border:view===v.id?"none":`1px solid ${T.divider}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:view===v.id?600:400}}>{v.l}</button>)}
      {view==="calendar"&&<button onClick={()=>sSNT(true)} style={{background:T.accentSoft,border:`1px solid ${T.divider}`,color:T.textSec,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:12}}>+ 标签</button>}
  ...
  const GL=<><style>{`@keyframes slideIn{from{transform:translateX(80px);opacity:0}to{transform:translateX(0);opacity:1}}*{-webkit-tap-highlight-color:transparent}`}</style>{toasts.map(t=><Toast key={t.id} msg={t.msg} onClose={()=>sToasts(p=>p.filter(x=>x.id!==t.id))}/>)}</>;
  const wrap={maxWidth:920,margin:"0 auto",padding:"16px 16px 80px"};

  // ── TAG VIEW
  if(view==="tags"){const ents=Object.entries(data).filter(([_,v])=>v.todos?.some(t=>t.tags?.includes(fTag))).sort(([a],[b])=>b.localeCompare(a));const tO=tags.find(t=>t.id===fTag);
    return <div style={{minHeight:"100vh",background:T.bg}}>{GL}<div style={wrap}><Nav/>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:20}}>{tags.map(t=><Pill key={t.id} active={fTag===t.id} color={t.color} onClick={()=>sFTag(t.id)}>{t.icon} {t.label}</Pill>)}</div>
      {!ents.length?<div style={{textAlign:"center",padding:50,color:T.textTer}}><p>暂无「{tO?.label}」记录</p></div>:<div style={{display:"flex",flexDirection:"column",gap:12}}>{ents.map(([d,e])=><EntryCard key={d} dateKey={d} e={e} hl={tO?.color} tags={tags}/>)}</div>}
    </div></div>}

  // ── MOOD VIEW
  if(view==="mood"){const ents=Object.entries(data).filter(([_,v])=>v.mood===fMood).sort(([a],[b])=>b.localeCompare(a));const mO=MOODS.find(m=>m.val===fMood);
    return <div style={{minHeight:"100vh",background:T.bg}}>{GL}<div style={wrap}><Nav/>
      <div style={{display:"flex",gap:8,marginBottom:20}}>{MOODS.map(m=><button key={m.val} onClick={()=>sFMood(m.val)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,background:fMood===m.val?T.card:T.accentSoft,border:fMood===m.val?`2px solid ${m.color}`:`1px solid ${T.divider}`,borderRadius:T.radius,padding:"12px 4px",cursor:"pointer",boxShadow:fMood===m.val?T.shadowMd:"none"}}>
        <span style={{fontSize:26}}>{m.emoji}</span><span style={{fontSize:11,color:fMood===m.val?m.color:T.textTer,fontWeight:500}}>{m.label}</span>
        <span style={{fontSize:18,fontWeight:700,color:fMood===m.val?T.text:T.textTer}}>{Object.values(data).filter(v=>v.mood===m.val).length}</span>
      </button>)}</div>
      {!ents.length?<div style={{textAlign:"center",padding:50,color:T.textTer}}><p>暂无「{mO?.label}」记录</p></div>:<div style={{display:"flex",flexDirection:"column",gap:12}}>{ents.map(([d,e])=><EntryCard key={d} dateKey={d} e={e} hl={mO?.color} tags={tags}/>)}</div>}
    </div></div>}

  // ── SCORE VIEW
  if(view==="score"){const ranges=[[9,10,"卓越","#059669"],[7,8,"不错","#2563eb"],[5,6,"一般","#d97706"],[3,4,"低落","#ea580c"],[1,2,"艰难","#dc2626"]];
    const[lo,hi]=fScore||[8,10];const ents=Object.entries(data).filter(([_,v])=>v.score>=lo&&v.score<=hi).sort(([a],[b])=>b.localeCompare(a));const rO=ranges.find(r=>r[0]===lo&&r[1]===hi);
    return <div style={{minHeight:"100vh",background:T.bg}}>{GL}<div style={wrap}><Nav/>
      <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>{ranges.map(([rl,rh,lb,cl])=>{const a=lo===rl&&hi===rh;const cnt=Object.values(data).filter(v=>v.score>=rl&&v.score<=rh).length;
        return <button key={rl} onClick={()=>sFScore([rl,rh])} style={{flex:1,minWidth:70,display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:a?T.card:T.accentSoft,border:a?`2px solid ${cl}`:`1px solid ${T.divider}`,borderRadius:T.radius,padding:"10px 6px",cursor:"pointer",boxShadow:a?T.shadowMd:"none"}}>
          <span style={{fontSize:12,color:a?cl:T.textSec,fontWeight:600}}>{lb}</span><span style={{fontSize:10,color:T.textTer}}>{rl}-{rh}</span>
          <span style={{fontSize:18,fontWeight:700,color:a?T.text:T.textTer}}>{cnt}</span>
        </button>})}</div>
      {!ents.length?<div style={{textAlign:"center",padding:50,color:T.textTer}}><p>暂无记录</p></div>:<div style={{display:"flex",flexDirection:"column",gap:12}}>{ents.map(([d,e])=><EntryCard key={d} dateKey={d} e={e} hl={rO?.[3]} tags={tags}/>)}</div>}
    </div></div>}

  // ── MONTHLY
  if(view==="monthly"){const am=Object.keys(stats.ms).sort().reverse();
    return <div style={{minHeight:"100vh",background:T.bg}}>{GL}<div style={wrap}><Nav/>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>{am.map(ym=>{const[y,m2]=ym.split("-");return <Pill key={ym} active={sMonth===ym} onClick={()=>sSMonth(ym)}>{y}年{MO[parseInt(m2)-1]}</Pill>})}</div>
      <SummaryView sm={stats.ms[sMonth]} tags={tags} label="本月"/>
    </div></div>}

  // ── YEARLY
  if(view==="yearly"){const ay=Object.keys(stats.ys).sort().reverse();
    return <div style={{minHeight:"100vh",background:T.bg}}>{GL}<div style={wrap}><Nav/>
      <div style={{display:"flex",gap:6,marginBottom:20}}>{ay.map(y=><Pill key={y} active={sYear===y} onClick={()=>sSYear(y)}>{y}年</Pill>)}</div>
      <SummaryView sm={stats.ys[sYear]} tags={tags} label="本年"/>
    </div></div>}
    ...
    <div style={wrap}><Nav/>
      <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
          ...
          <div style={{background:T.card,borderRadius:16,padding:22,border:`1px solid ${T.cardBorder}`,boxShadow:T.shadow}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
              ...
              {Array.from({length:dim},(_,i)=>i+1).map(d=>{
                const dK=dkFn(yr,mo,d);const isT=dK===todayK;const isS=dK===sel;const has=!!data[dK];const dots=getDots(dK);
                const mE=data[dK]?.mood?MOODS.find(m=>m.val===data[dK].mood)?.emoji:null;
                return <button key={d} onClick={()=>{sSel(dK);sEDiary(false);sETIdx(null)}} style={{
                  ...
                  {dots.length>0&&<div style={{display:"flex",gap:2,position:"absolute",bottom:3}}>{dots.map((c,i)=><div key={i} style={{width:4,height:4,borderRadius:"50%",background:isS?"rgba(255,255,255,0.6)":c}}/>)}</div>}
                </button>
              })}
            </div>
            <div style={{marginTop:14,display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center"}}>
              {tags.map(t=><span key={t.id} style={{fontSize:10,color:T.textTer,display:"flex",alignItems:"center",gap:3}}><span style={{width:5,height:5,borderRadius:"50%",background:t.color,display:"inline-block"}}/>{t.label}</span>)}
            </div>
          </div>
        </div>
        <div style={{flex:"1 1 380px",minWidth:300}}>
          ...
          <div style={{background:T.card,borderRadius:16,padding:22,border:`1px solid ${T.cardBorder}`,boxShadow:T.shadow,maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <div style={{marginBottom:20}}>
              <div style={{fontSize:12,color:T.textTer,marginBottom:8,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.03em"}}>待办事项 {entry?.todos?.length>0&&<span>{entry.todos.filter(t=>t.done).length}/{entry.todos.length}</span>}</div>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(entry?.todos||[]).map((todo,i)=><div key={todo.id||i} style={{background:T.accentSoft,borderRadius:10,padding:"10px 12px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <button onClick={()=>handleToggle(i)} style={{width:20,height:20,borderRadius:6,border:todo.done?"none":`1.5px solid ${T.textTer}`,background:todo.done?T.text:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,flexShrink:0}}>{todo.done&&"✓"}</button>
                    ...
                    <button onClick={()=>sETIdx(i)} style={{background:"none",border:"none",color:T.textTer,cursor:"pointer",fontSize:12}}>✏️</button>
                    <button onClick={()=>handleRemove(i)} style={{background:"none",border:"none",color:T.textTer,cursor:"pointer",fontSize:14}}>×</button>
                  </div>
                ...
                <div style={{display:"flex",gap:8}}>
                  <input value={nTodo} onChange={e=>sNTodo(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAddTodo()} placeholder="添加待办..." style={{flex:1,background:T.accentSoft,border:`1px solid ${T.divider}`,borderRadius:T.radiusSm,padding:"9px 12px",color:T.text,fontSize:13,outline:"none"}}/>
                  <button onClick={handleAddTodo} style={{background:T.text,border:"none",borderRadius:T.radiusSm,padding:"9px 16px",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600}}>+</button>
                </div>
                <p style={{margin:0,fontSize:11,color:T.textTer}}>添加后点 ✏️ 关联标签和设置提醒</p>
              </div>
            </div>
            <div style={{marginBottom:20}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={{fontSize:13,color:T.text,fontWeight:500,fontStyle:"italic"}}>Say Something ✨</span>
              ...
              {eDiary?<textarea value={entry?.diary||""} onChange={e=>updateEntry(sel,{diary:e.target.value})} placeholder="写下今天的想法..." style={{width:"100%",minHeight:100,background:T.accentSoft,border:`1px solid ${T.divider}`,borderRadius:10,padding:12,color:T.text,fontSize:13,lineHeight:1.8,resize:"vertical",outline:"none",boxSizing:"border-box",fontFamily:"inherit"}}/>
              :<div onClick={()=>sEDiary(true)} style={{background:T.accentSoft,borderRadius:10,padding:14,minHeight:50,cursor:"pointer",fontSize:13,lineHeight:1.8,color:entry?.diary?T.textSec:T.textTer}}>{entry?.diary||"点击写点什么..."}</div>}
            </div>
            <div style={{marginBottom:20}}>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                ...
                <span style={{fontSize:20,fontWeight:700,color:T.text}}>{entry?.score||5}<span style={{fontSize:13,color:T.textTer,fontWeight:400}}>/10</span></span>
              </div>
